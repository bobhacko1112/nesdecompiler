...
static chr_page_size: usize = 8192;
static prg_page_size: usize = 8192 *2;

fn main() -> io::Result<()> {
    let mut byte_count = 0;
    let mut args = env::args();
    if args.len() !=2 {
        println!("Usage: $ nes_dc <nes_rom_filename.rom>\nSpits out prg.asm and chrXX.ppm");
        return Ok(());
    }
    args.next();
    let mut input_file = File::open(args.next().unwrap())?;
    let mut bytes = Vec::new();
    for each_byte in input_file.bytes() {
        bytes.push(each_byte);
    }
    let stamp: Vec<Result<u8, u8>> = vec![Ok(78u8), Ok(69u8), Ok(83u8)];

    for (a,b) in bytes[0..3].iter().zip(stamp.iter()) {
        if (a.as_ref().unwrap() != b.as_ref().unwrap()) {
            println!("File corrupt or incorrect file type");
            return Ok(());
        }
    }
    if bytes.len() < 8192 {
    ...
    for (i, byte) in bytes {
       ...
       if i < header_size {
           if i == 4 {
               match byte {
                   Ok(data) => {prg_pages = *data}
                   Err(e) => {
                       println!("File header corrupt: {}", e);
                       break}
               }
           }
           if i == 5 {
               match byte {
                   Ok(data) => {chr_pages = *data}
                   Err(e) => {
                       println!("File header corrupt: {}", e);
                       return Ok(())}
               }
           ...
           if i == 6 {
               match byte {
                   Ok(data) => {
                       println!("{:b}", data);
                        let mirroring_h = data & 0b00000001;
                        let mirroring_v = data & 0b00000010;
                        let battery = data & 0b00000100;
                        let four_screen = data & 0b00001000;
                        let mapper_num = data & 0b11110000;
                        println!("mirroring_h: {}, mirroring_v: {}, battery: {}, four_screen: {}, mapper_num: {}", mirroring_h, mirroring_v, battery, four_screen, mapper_num);
                   }
                   Err(e) =>{
                       println!("File header corrupt: {}", e);
                       break}
       ...
       if code_size > 1 {
        ...
        if i > header_size && i < prg_pages as usize *prg_page_size {
            match byte {
                Ok(data) => {
    ...
    println!("// CHR_Pages:{}  PRG_pages:{} Total Bytes: {}", chr_pages, prg_pages, byte_count);
   Ok(())
}
